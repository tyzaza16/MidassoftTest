export function getQuestionPart(phrases: string[]): string[] {
  ...
  const commonWord: string = findCommonWord(phrases);
...
function findCommonWord(phrases: string[]): string {

  let answerWord: string = '';

  let keyword: string = ''; 
  let keywordInverted: string = '';
  let keywordFlag: boolean = true;
  let keywordInvertedFlag: boolean = true;
  ...
  for(let i = 0 ; i < phrases[0].length ; i++) {
    ...
    for(let j = 1 ; j < phrases.length ; j++) { 
      ...
      if(
          (phrases[j].indexOf(keywordInverted)) >= 0 && 
